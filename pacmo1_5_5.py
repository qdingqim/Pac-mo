"""
CS 177 Group 17 Project Pac-mo Version 1.5.5
...
Todo:
    ...
    Terminate at max reward
        - terminate if we get reward of 27.
...
def end_mission(agent_hosts, death, last):
    if death:
        update_q_table(last[0], last[1], -100)
...
def choose_action(curr_coord, grid, last):
    ...
    if last[1] != "" and last[1] in q_table[curr_coord]:
        if q_table[last[0]][last[1]] > 0 and q_table[curr_coord][last[1]] > 0:
            i = 0
...
def player_action(last, block_coord, turn_count, yaw, grid):
    ...
    direction, to_where = choose_action(block, grid, last)
